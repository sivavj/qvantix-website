...
export const WhyQvantixSection: React.FC = () => {
  const getIcon = (idx: number) => {
    switch (idx) {
      case 0:
        return <Target className="w-6 h-6 text-[#307e3e]" />;
      case 1:
        return <Layers className="w-6 h-6 text-[#307e3e]" />;
      case 2:
        return <Clock className="w-6 h-6 text-[#307e3e]" />;
      case 3:
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end mb-16">
          ...
          <div className="lg:col-span-4 lg:text-right">
            <div className="p-4 rounded-xl bg-[#307e3e]/10 border border-[#307e3e]/30 inline-block text-left shadow-sm">
              <span className="text-xs font-mono text-[#307e3e] font-bold block mb-1">
                OUR CORE BELIEF
              </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {WHY_QVANTIX.map((item, idx) => (
            <div
              key={item.number}
              className="light-card light-card-hover rounded-2xl p-7 flex flex-col justify-between space-y-6 relative overflow-hidden hover:border-[#f6921e]"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 rounded-xl bg-[#307e3e]/10 border border-[#307e3e]/30 flex items-center justify-center">
                    {getIcon(idx)}
                  </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                <span className="text-xs font-mono font-bold text-[#307e3e] flex items-center gap-1">

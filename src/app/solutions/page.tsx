        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {SERVICES.map((service, index) => (
              <div
                key={service.id}
                className="bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col justify-between space-y-6 group hover:shadow-xl hover:border-[#f6921e] transition-all duration-300"
              >
                ...
                <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-900 border-b border-slate-100">
                  ...
                  <div className="absolute bottom-3 right-3 bg-slate-900/90 text-slate-200 text-[10px] font-mono px-2.5 py-1 rounded-md border border-slate-700 backdrop-blur-md">
                    {service.category}
                  </div>
                </div>

                <div className="p-6 sm:p-8 pt-0 space-y-6 flex-1 flex flex-col justify-between">
                  ...
                  <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
                    <div className="flex flex-wrap gap-1.5">
                      {service.technologies.slice(0, 3).map((t) => (
                        <span key={t} className="text-[10px] font-mono text-[#307e3e] bg-[#307e3e]/10 px-2 py-0.5 rounded border border-[#307e3e]/30 font-bold">
                          {t}
                        </span>
                      ))}
                    </div>
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#307e3e] hover:bg-[#246330] text-white text-xs font-bold transition-all shrink-0 shadow-sm"
                    >
